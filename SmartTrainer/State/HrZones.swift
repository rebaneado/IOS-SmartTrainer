...
// The rider's heart-rate zones (BPM), from their TrainingPeaks setup. These are
// display-only — the workouts are driven by power, not HR — but they let the
// ride screen colour the live BPM by zone and the summary show time-in-zone.
//
...
enum HrZones {
    static let all: [HrZone] = [
        HrZone(index: 1, name: "Warm Up", short: "Z1", minBpm: 0, maxBpm: 118,
               color: Color(red: 0.427, green: 0.655, blue: 0.925)),   // #6da7ec
        HrZone(index: 2, name: "Easy", short: "Z2", minBpm: 119, maxBpm: 138,
               color: Color(red: 0.106, green: 0.686, blue: 0.478)),   // #1baf7a
        HrZone(index: 3, name: "Aerobic", short: "Z3", minBpm: 139, maxBpm: 158,
               color: Color(red: 0.929, green: 0.631, blue: 0.0)),     // #eda100
        HrZone(index: 4, name: "Threshold", short: "Z4", minBpm: 159, maxBpm: 178,
               color: Color(red: 0.922, green: 0.408, blue: 0.204)),   // #eb6834
        HrZone(index: 5, name: "Maximum", short: "Z5", minBpm: 179, maxBpm: Int.max,
